...
void bsp(Node st, Node en, int dis) {
	...
	mid.x = (st.x + en.x) / 2;
	mid.y = (st.y + en.y) / 2;
	if (abs(en.x - st.x) * abs(en.y - st.y) < 400 || (abs(en.y - st.y) < 15 || abs(en.x - st.x) < 15)) {
		...
		flag = rand() % 2;
		if (abs(en.y - st.y) < 15 || dis < 0 || flag == 0) {
			//if (en.x - st.x > 8) mid.x = mid.x + (rand() % 5 - 1) * (en.x - st.x) / 10;
		...
		else if (abs(en.x - st.x) < 15|| dis > 0  || flag == 1) {
			//if (en.y - st.y > 8)mid.y = mid.y + (rand() % 5 - 1) * (en.y - st.y) / 10;
